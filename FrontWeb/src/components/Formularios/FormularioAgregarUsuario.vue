...
export default {
    ...
    methods: {
        async onSubmit(valores) {
            ...
            if (!this.administrador) {//USUARIO
                ...
                const response = await axios.post(import.meta.env.VITE_API + "/api/usuarios/adm/agregarAdministrador", data, 
                        {headers: {
    ...
    emits: ['cerrarDialogo', 'usuarioAgregado']
}
</script>
    ...
    <Form @submit="onSubmit">
        <div id="seccionControl">
            ...
            <div id="contenedorControl" style="display: flex; align-items: center; justify-content: left;">
                ...
                <v-checkbox hide-details v-model="administrador"
                    style="color: var(--color-primario); font-size: 23px; margin-left: 10px;"></v-checkbox>
            </div>
            <p id="errorControl">{{ errorPostSubmit }}</p>
        </div>
        <Boton :loading="procesando" tipo="primario2" type="submit">Crear usuario</Boton>
