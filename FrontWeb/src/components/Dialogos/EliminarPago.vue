...
export default {
    ...
    methods: {
        ...
        async eliminarPago() {            
            const url = import.meta.env.VITE_API + "/api/pagos/usf/deletePago/" + this.pagoId
        ...
        <v-card style="padding: 10px; padding-top: 20px; padding-bottom: 20px;">
            <template v-if="!eliminado">
                <v-card-text style="padding-bottom: 20px;">
                    Seguro/a que desea eliminar este pago?
                </v-card-text>
                <v-card-actions style="padding-left: 22px;">
                    <Boton style="margin-right: 10px;" tipo="primario2" @click="eliminarPago()">Eliminar</Boton>
